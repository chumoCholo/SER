...
function main(query){
    let arr = fetch("Titulos.php",{
        ...
        return response.json();
         ...
         let i= 0;
         let elements = [];
        for (let index = 2; index <  Object.keys(data).length*2+1; index++) {
            
        if(data[index]!= null){
            
           
            elements[i++] = data[index];
        }
        }
        let resultList = document.querySelector(".result");
        resultList.innerHTML = "";
    
        elements.map(function(algo){
            query.split(" ").map(function (word){
                if(algo.toLowerCase().indexOf(word.toLowerCase()) != -1){
                    string  = new String(algo);
                    string  = string.replace(".html","");
                    resultList.innerHTML += `<a class="list-group-item" href="/Proyecto/publicaciones/${algo}">${string}</a>`;
                }
            })
        })
